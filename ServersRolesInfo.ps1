...
Begin
{
    ...
    ####################################### Check the Host properties #####################################################
    function Check-HostProperties
    {
        Param
        (
            $InData
        ...
        try
        {
            Write-LogFile -LogText "Checking whether the server is part of domain or not."
            $Domain = Get-WmiObject -Class Win32_ComputerSystem
            if($Domain.PartOfDomain)
            {
                Write-LogFile -LogText "The current host is part of domain."
                #return
            }
            else
            {
                Write-LogFile -LogText "The current server is not part of domain i.e is in WorkGroup. So adding the target servers to TrustedHosts."
                $AllNames = $InData.ServerNames
                $ServerString = $AllNames -join ","
                $ExistingTrsutedHosts = (Get-Item WSMan:\localhost\Client\TrustedHosts).Value
                if($ExistingTrsutedHosts)
                {
                    $ExistingTrsutedHosts = $ExistingTrsutedHosts+","+$ServerString
                    $raw = Set-Item -Path WSMan:\localhost\Client\AllowUnencrypted -Value $true -Force
                    $raw = Set-Item -Path WSMan:\localhost\Client\TrustedHosts -Value $ExistingTrsutedHosts -Force
                    Write-LogFile -LogText "Added the servers to the Trusted Hosts list"
                }
                else
                {
                    $raw = Set-Item -Path WSMan:\localhost\Client\AllowUnencrypted -Value $true -Force
                    $raw = Set-Item -Path WSMan:\localhost\Client\TrustedHosts -Value $ServerString -Force
                    Write-LogFile -LogText "Added the servers to the Trusted Hosts list"
                }
            }
        }
        ...
        {
            Write-LogFile -LogText "There was an exception while checking the current host details and adding the servers to trusted hosts list. $($error[0].Exception.Message)"
            Write-Host -ForegroundColor Red "There was an exception while checking the current host details and adding the servers to trusted hosts list. $($error[0].Exception.Message)"
            exit
        }
    }
    ...
    function Get-FileShareSizes
    {
        ...
        {   
            ...
            {
                Write-LogFile -LogText "There was an exception while fecthing the File Share information: $($error[0].Exception.Message)"
                return $null
            }
        ...
        {
            Write-LogFile -LogText "Exception while fetching file share size information on $ServerInfo : $($error[0].Exception.Message)"
        ...
        {   
            ...
            {   		
                ...
                if($Server -eq $hostname)
                {
                    ...
                    $InstalledApplications = Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\* | Select-Object -Property DisplayName, DisplayVersion,Publisher
                }
                else
                {
                    ...
                    $InstalledApplications = Invoke-Command -ComputerName $Server -Credential $Creds -ScriptBlock {
                        Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*
                    }
                    $InstalledApplications = $InstalledApplications | Select-Object -Property DisplayName, DisplayVersion,Publisher
                }
            }
            catch
            {
                Write-LogFile -LogText "There was an exception while fetching installed applications: $($error[0].Exception.Message)"
                return $null
            }
            ...
            if($InstalledApplications)
            {
                ...
                foreach($app in $InstalledApplications)
                {
                    ...
                    {
                        $EachApp = @{}
                        $EachApp.version = $app.DisplayVersion
                        $EachApp.manufacturer = $app.Publisher
                        $AppCollection[$app.DisplayName] = $EachApp
                    }
                }
            }
            else
    ...
    Function Get-Roles
    {
        ...
        try
        {
            ...
            {
                $AllRoles  = Get-WindowsFeature #| Where-Object {$_.FeatureType -eq 'Role'}
            }
            else
            {
                $AllRoles  = Get-WindowsFeature -ComputerName $ServerName -Credential $Creds #| Where-Object {$_.FeatureType -eq 'Role'}
            }
            return $AllRoles
        }
        catch
        {
            Write-LogFile -LogText "There was an exception while fetching the roles. $($error[0].exception.message)"
            return $null
        }
    }
    ...
    Function Get-InstalledPrintServerRole
    {
        ...
        try
        {
            Write-LogFile -LogText "Fetching Print Server roles on $($server.ServerNames)" 
            
            $PrintServices = $InRoles | Where-Object {$_.FeatureType -eq 'Role'} | Where-Object {$_.Name -eq 'Print-Services'}

            # Hash Object to hold the key-value pairs
            $RolesObj = @{}
            # Checking if the Print server roles are available
            if($PrintServices -ne $null)
            {
        ...
        {
            Write-LogFile -FilePath $LogFilePath -LogText "There was an exception while fetching print server information on $($server.ServerNames).$($error[0].exception.message)"
            $Script:ExceptionCodes += "Exception"
            return $null
        }
    }
    ...
    Function Get-InstalledFileServerRole
    {
        ...
        try
        {
            Write-LogFile -LogText "Fetching File Server roles on $($server.ServerNames)" 

            $FileServices = $InRoles | Where-Object {$_.FeatureType -eq 'Role'}| Where-Object {$_.Name -eq 'FileAndStorage-Services'}
            ...
            if($FileServices -ne $null)
            {
                ...
                if($FileServices.InstallState -eq 'Installed')
                {
                    ...
                    $RolesObj['FileAndStorage-Services'] = 'Yes'
                ...
                else
                {
                    ...
                    $RolesObj['FileAndStorage-Services'] = 'No'
                }
                #$AllCollections += $o
            }
            else
        ...
        {
            Write-LogFile -LogText "Exception while fetching file server information on $($server.ServerNames) : $($error[0].exception.message)"
            $Script:ExceptionCodes += "Exception"
            return $null
        }
    }
    ...
    try
    {
        ...
        if(Test-Path -Path $CSVFilePath)
        {
            ...
            if($CSVdata)
            {
                ...
                # Function call to check the current host details i.e workgroup or domain joined
                Check-HostProperties -InData $CSVdata
                ...
                foreach($server in $CSVdata)
                {
                    ...
                    if($ServerStatus.StatusCode -eq 0)
                    {
                        ...
                        $AllRolesObj = @{}
                        $RolesInServer = Get-Roles -ServerName $server.ServerNames -Creds $Credentials
                        # Function call to fetch RDS Roles information
                        #$RDSRoles = Get-InstalledRDSRoles -Server $server.ServerNames -Creds $Credentials
                        $RDSRoles = Get-InstalledRDSRoles -InRoles $RolesInServer
                        $AllRolesObj += $RDSRoles
                        ...
                        $PrintRoles = Get-InstalledPrintServerRole -InRoles $RolesInServer
                        $AllRolesObj += $PrintRoles
                        ...
                        $FileServerRoles = Get-InstalledFileServerRole -InRoles $RolesInServer
                        $AllRolesObj += $FileServerRoles
                        $JSONObj | Add-Member -MemberType NoteProperty -Name roles -Value $AllRolesObj
